		...
		if len(abb.keys()) > 0:
			...
			if len(sus) > 0:
				...
				for a in sus:
					...
					#try:
					m = max(starmap(pw2,itertools.product([seq],als)))
					if m < 0.15:
...
def run(sys_argv=sys.argv):
	...
	def opensplitFile(path):
		...
		return data
	#
	#sys_argv=['', '/home/musellla/txtmining/tmp-MEL_D3NER/MEL_D3NER_subgraphs_expansion1/', 'MEL_D3NER_subgraphs_expansion1', '32']
	#
	wd= sys_argv[1] # tmp
