...
def edge_list(kw1,kw2,keys_pub,N,path=sys.argv[1]+"data/"+sys.argv[2]+"_edge_list_allxall.tsv"):
		#
		#N=len(pmids)
		N=globals()[N]
		#kw1,kw2=kws
		...
		if any((x <= 0 for x in [(R1C1+R2C1),(R1C2+R2C2),(R1C1+R1C2),(R2C1+R2C2)])):
			...
			edge_weight=fisher_exact(table, alternative='greater')[1] # P_ba*P_ab
		#
...
#
def approxurn(Aij,csi,vseq,M,m):
	csi=globals()[csi]
	vseq=globals()[vseq]
	M=globals()[M]
	m=globals()[m]
...
def run(sys_argv=sys.argv):
	...
	#
	wd=sys_argv[1] #'/home/musellla/tam_textmining/'
	sd= re.split("tmp",wd)[0]
	tag=sys_argv[2] #"iter1"
	hd = re.split("\/"+tag,wd)[0]+'/'
	ncores= int(sys_argv[3]) #32 #round(lim/tot)
	...
	# save lables to apply to nodes laterit=sys_argv[4]
	it=sys_argv[4]
	...
	var_names_in=["pmids_"+str(i) for i in range(ncores)]

	for name in var_names_in:
		globals()[name] = len(pmids)
	...
	def distro(datal,ncores,d1,d2):
		...
		return iter((datal[i][0],datal[i][1],d1s[i],d2s[i]) for i in range(0,len(datal)))
	#
	start=time.time()
	with Pool(ncores) as pool:
		...
		print("starmap...")
		abs_dict=pool.starmap(edge_list,distro(combs,ncores,'dict_pub_','pmids_'))
	...
	var_names_in=["M_"+str(i) for i in range(ncores)]
	...
	if math.isinf(scipy.special.comb(M,m)):
		...
		with Pool(ncores) as pool:
			#res=[pool.apply(find_in_series,args=(idd,globals()["abs_dict_"+str(ids.index(idd))],globals()["syn_dict_"+str(ids.index(idd))])) for idd in ids]   
			print("starmap...")
			stats=pool.starmap(approxurn,distro(list(A.items()),ncores,'csi_','vseq_','M_','m_'))
		...
		with Pool(ncores) as pool:
			#res=[pool.apply(find_in_series,args=(idd,globals()["abs_dict_"+str(ids.index(idd))],globals()["syn_dict_"+str(ids.index(idd))])) for idd in ids]   
			print("starmap...")
			stats=pool.starmap(hyperurn,distro(list(A.items()),ncores,'csi_','vseq_','M_','m_'))
			print("... done in %2f seconds" % (time.time()-start))
	#
	# ADJUST PVALUES
