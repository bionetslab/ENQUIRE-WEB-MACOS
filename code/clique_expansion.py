...
def run(sys_argv=sys.argv):
	...
	wd=sys_argv[1]
	#
	tag=sys_argv[2] #iter1
	#
	sd=re.split("tmp",wd)[0]
	#tmp=re.split(tag+"\/",wd)[0]
	tmp=re.split("\/"+tag,wd)[0]+'/'
	#
	prev=sys_argv[3] #"vcells"
	prevwd=tmp+prev+"/"
	#
	sd=re.split("tmp",wd)[0]
	...
	#
	Network=nx.from_pandas_edgelist(exp_edge_tbl,'kw1','kw2',['pg','crit_p','edge_weight','inv_edge_weight','R1C1','iteration'])
	# adding node attributes
	nx.set_node_attributes(Network,attr)
	#del attr
	### the previous, complete network
	# load the "complete graph"
	comp_node_tbl=pd.read_csv(prevwd+prev+"_Complete_nodes_table_subgraph.tsv",delimiter='\t')
	comp_edge_tbl=pd.read_csv(prevwd+prev+"_Complete_edges_table_subgraph.tsv",delimiter='\t')
	#
	...
	del attr
	#
	ents=comp_node_tbl['KW'].tolist()
	############
	# COMPARE CLIQUES, T0 VS T1 
	def maximal_cliques(G, n):
	    ...
	    for clique in nx.find_cliques(G):
	        if len(clique) <= n and len(clique)>=3:
	            mcs.append(clique)
	    return mcs
	#
	PCs=maximal_cliques(comp_Net,len(comp_Net.nodes()))
	NCs=maximal_cliques(Network,len(Network.nodes()))
	# 
	def isexp(nc,PCs):
		return any([all([e in nc for e in pc]) for pc in PCs])
	#
	### KEEP NODES WITH NEIGHBOURS IN MORE THAN ONE OF THE PREVIOUS COMMUNITIES ###
	clust_prop=pd.read_csv(prevwd+prev+"_nodes_in_clique-clique_communities_net.tsv",sep='\t')
	clust=pd.Series(clust_prop.membership)
	clust.index=clust_prop.entity.tolist()
	### here insert user parameter ### 
	tight=int(sys_argv[4])
	print("nodes bridging at least %i of the previous communities will be added:" % (tight))
	#
	def is_clot(g,tight):
		l=len(set(clust[clust.index.isin([n for n in Network.neighbors(g)])]))
		return l>=tight
	#
	# KEEP CLIQUES THAT ARE SUPERSETS OF PREVIOUS ONES
	#NCs=[nc for nc in NCs if isexp(nc,PCs)]
	addgenes=[k for k in Network.nodes() if is_clot(k,tight)]
	#
	#addgenes=set(itertools.chain.from_iterable(NCs))
	############
	print("found %i new connecting entities" % (len(set(addgenes) - set(comp_Net.nodes()))))
	#
	keeNet=Network.subgraph(addgenes)
	#
	...
	#
	Network=nx.from_pandas_edgelist(pre_edge_tbl,'kw1','kw2',['pg','crit_p','edge_weight','inv_edge_weight','R1C1','iteration'])
	#
	...
	edge_tbl[['kw1','kw2','pg','crit_p','edge_weight','inv_edge_weight','R1C1','iteration']].to_csv(wd+tag+"_Complete_edges_table_subgraph.tsv",sep='\t',index=False)
	# 
	...
	mesh_edge_tbl[['kw1','kw2','pg','crit_p','edge_weight','inv_edge_weight','R1C1','iteration']].to_csv(wd+tag+"_MeSH_edges_table_subgraph.tsv",sep='\t',index=False)
	#
	...
	gene_edge_tbl[['kw1','kw2','pg','crit_p','edge_weight','inv_edge_weight','R1C1','iteration']].to_csv(wd+tag+"_Genes_edges_table_subgraph.tsv",sep='\t',index=False)
	#
