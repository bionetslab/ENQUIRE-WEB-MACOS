...
for (data in names(NetS)){
  ...
  gc(verbose=F)
  free=as.numeric(system("vm_stat | grep -e 'Pages free' | sed 's/[^0-9]*//g'", intern=TRUE))
  pagesize=as.numeric(system("vm_stat | grep -e 'page size of' | sed 's/[^0-9]*//g'", intern=TRUE)) # pagesize in bytes 
  free=pagesize*free/1E3 # KB
  #free=as.numeric(system("awk '/MemFree/ {print $2}' /proc/meminfo", intern=TRUE)) # available memory at that time
  use=0.25*free
  print(sprintf("set to use around 25%% of the available RAM, i.e. %g GB", signif(use/1E6,digits=2)))  
  net=NetS[data][[1]]
  g.tm=net$graph
  B=net$B
